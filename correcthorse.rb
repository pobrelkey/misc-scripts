...
word_count = ((bit_count*Math.log(2))/Math.log(words.size)).ceil
byte_count = ((word_count*Math.log(words.size))/Math.log(256)).ceil
bits = File.open('/dev/urandom','rb'){|f| f.read(byte_count) }.chars.inject(0){|a,b| a << 8 | b.ord }
puts (0...word_count).collect{|e| words[(bits / (words.size ** e)) % words.size] }.join(' ')
